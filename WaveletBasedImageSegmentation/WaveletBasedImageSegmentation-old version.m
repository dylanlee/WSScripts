...
% (c) 2014 - Ashutosh Kumar Upadhyay  & Ratnesh Singh Sengar
%
clear all
display 'program starts...'
n = 512;
%name='syn_new_9_';  %syn_16_40_3_';%b10nature_trees000_';
%Load image
I =imread('syn_new_9.bmp'); %syn_16_20_3.bmp'); %100_0109.png'); 
I=imresize(I,[n n]);
%I=I(1:n,1:n);
%ground truth image if available
Igt=I; %imread('syn_new_9_gt.bmp');
In=I;
...
if (IsGelImage == 1)
	...
	MorphSmoothRad=1;   %morphological disk radius set to 1 as per paper if image is noisy, other wise it can be set to 0.
	smallRadiusofSpot=5;  % required in case of 2D gel image.
else
      %%%paramters tested for natural images available at http://www.wisdom.weizmann.ac.il/~vision/Seg_Evaluation_DB  ( for biomodal segmentation)
	...
	ContrastThreshold_Th = 70;   % high value means detect high contrast objects
	MorphSmoothRad=0;   %morphological disk radius set to 1 as per paper if image is noisy, other wise it can be set to 0.
	smallRadiusofSpot=1;  % required in case of 2D gel image.
...
I=I;
mm1=I;%mm=floor(255*M);
mm=I(:,:,1);mm(e>0)=255;mm1(:,:,1)=mm;mm=I(:,:,2);mm(e>0)=0;mm1(:,:,2)=mm;mm=I(:,:,3);mm(e>0)=0;mm1(:,:,3)=mm;figure,imshow(mm1),title('final');
