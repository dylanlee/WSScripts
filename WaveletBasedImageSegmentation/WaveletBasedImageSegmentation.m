% This implements the code of our work. The work is recently submitted as a paper "Wavelet Based BiModal Image segmentation for Blob Extraction...".
%Till now, you can cite this program as a open source code available at 
% http://in.mathworks.com/matlabcentral/fileexchange/48610-wavelet-based-image-segmentation
%  This code is useful to segment out blob like objects of different intensities and shapes.
% This is also useful as a general purpose image segmentation methods and can be compared with all the state of the art methods.
%
%
% .
%
...
%  1. Toolbox Wavelets by Gabriel Peyre available at http://www.mathworks.com/matlabcentral/fileexchange/5104-toolbox-wavelets'
%  2. k-means clustering available at http://www.mathworks.com/matlabcentral/fileexchange/24616-kmeans-clustering/content/litekmeans/litekmeans.m
%
%  
...
n = 2000;
name='imDefMap';%'syn_new_9_'; %syn_16_20_3';% %gel270415_2_3_';%spots1_';%syn6_513_1024_';%iitgel1-1_';%syn_new_9_';%002cell_';% %somasekar1 (12)_';%syn_16_20_3_'; %dsc04575_';%b10nature_trees000_';%100_0109_';%gel1tiff_orig_';%image16_gauss35salt10_';%100_0109_40_3_';% 100_0109_';%image16_gauss40_';  %syn_16_40_3_';%
%Load image
I =imread('HighresCrVegTest.tif');%'syn_new_9.bmp'); %syn_16_20_3.bmp'); %syn_new_9.bmp'); %gel270415_2_3.bmp');%spots1.bmp');%syn6_513_1024_.bmp'); %iitgel1-1.bmp');%002cell.png');%somasekar1 (12)1.tif');%007cell.png');%dsc04575.png');%b10nature_trees000.png');%gel1tiff_orig.tiff');%image16_gauss35salt10.bmp'); %image16_gauss40.bmp'); %syn_new_9.bmp'); %spatho1.bmp');%002cell.png');%syn_16_20_3.bmp'); %100_0109.png'); 

%the below line is likely for when you have an all black/white region that
%is throwing off the resuls
%I(I == 255) = 160;

%cutI = I(1:1000,2016:12015);
%I=I(513:1024,513:1024);
%I=imresize(I,[n n]);
%I = I(1:512,4001:4512);
I=I(2000:3984,2000:3984);
...
if (IsGelImage == 0)
	...
	smallRadiusofSpot=0;  % required in case of 2D gel image.
     ...
     %%2d gel images %%defaults: 6,6,3,5 %IMPORTANT!! values used for White
     %%sands sections 2-9 for deformation density analysis: 6,45,2,5
	KernelBandwidth_h =6;   %LOCKED   % this parameter depends upon the contents of the image and noise. For a similar type of image, it will not vary and values can be found out through experimetns by varying it between 1 to 10.
	ContrastThreshold_Th = 45;   % high value means detect high contrast objects
	MorphSmoothRad=2;   %LOCKED %morphological disk radius set to smaller than small radius of spot.
	smallRadiusofSpot=50;  %LOCKED % required in case of 2D gel image.
...
for i=1:max(L(:))
   ...
   mm=M(idx); mm=mm(:); c1=0;c2=0; idx2=1;idx3=1;
    ...
    %http://www.mathworks.com/matlabcentral/fileexchange/24616-kmeans-clustering/content/litekmeans/litekmeans.m
    %
...
%    rm1=imdilate(rm1,se);
    ...
    L=watershed(M1);
    L = double(L);
    L=L.*(bb4>0); %%% Thanx to Justin Du for removing error for newer versions of MATLAB
    bb4=(L>0);
